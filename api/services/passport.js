				...
				return sails.models.user.findOne({
					...
					if (existingUser) {
						...
						return sails.models.passport.create(_.extend({
							user: user.id
						}, query)).fetch().then(function (passport) {
							saveProfile(profile, passport.provider, user.id);
							next(null, user);
						})['catch'](next);
					} else {
						return sails.models.user.create(user).fetch().then(function (_user) {
							user = _user;
							return sails.models.passport.create(_.extend({
								user: user.id
							}, query)).fetch();
...
var saveProfile = function saveProfile(profile, provider, user) {
	...
	return sails.models.socialprofile.findOne({
		...
		if (!p) {
			return sails.models.socialprofile.create(_.extend({
				user: user
			...
			return sails.models.socialprofile.update({
				user: user,
				provider: provider
			}, _.extend({
				user: user
